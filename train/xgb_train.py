...
TF-IDF 벡터화와 XGBoost를 사용하여 보이스피싱 탐지 모델을 학습합니다.
5회 실행의 평균 성능을 계산하고 평가합니다.

사용 예시:
    python train/xgb_train.py --data dataset/total_dataset.csv
"""
...
def save_model(model, vectorizer, model_path: str, vectorizer_path: str = None):
    ...
    if vectorizer_path:
        ...
        vectorizer_path = str(model_path).replace(".model", "_vectorizer.pkl")
        vectorizer_full_path = base_path / vectorizer_path
        joblib.dump(vectorizer, vectorizer_full_path)
        print(f"💾 벡터라이저 저장: {vectorizer_full_path}")
...
def main():
    """메인 함수"""
    parser = argparse.ArgumentParser(description="XGBoost 모델 학습 및 평가")
    parser.add_argument("--data", type=str, default="dataset/total_dataset.csv", help="데이터 파일 경로 (상대경로)")
    parser.add_argument("--model_path", type=str, default="model/xgb_voicephishing.model", help="모델 저장 경로 (상대경로)")
    ...
    parser.add_argument("--eta", type=float, default=0.1, help="학습률")
    ...
    wandb.init(project=args.wandb_project, name=args.wandb_name or f"tfidf_xgb_{args.num_runs}runs", config={
        "model": "XGBoost", "max_features": args.max_features, "ngram_range": (1, 2), "random_state": args.seed,
        "eta": args.eta, "max_depth": args.max_depth, "num_boost_round": args.num_boost_round,
